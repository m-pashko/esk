<@requirement.NODE 'sc-web' 'sc-web-${namespace}' 'sc-web-standby-${namespace}' />
...
<@requirement.PARAM name='IMAGENARIUM_API_URI' value='http://:5555/api/v4/components' description='URI API консоли управления' />
...
<@requirement.PARAM name='LDAP_ADMIN_PASSWORD' value='' description='Пароль администратора' />

<@img.TASK 'sc-web-${namespace}' '172.25.6.89:8123/web:${PARAMS.TAG}'>
  <@img.NODE_REF 'sc-web' />
  ...
  <#if PARAMS.ES_URL?has_content>
    <@img.ENV 'LOGSTASH_HOST' 'logstash-sc-web-${namespace}:4560' />
  </#if>
...
<#if PARAMS.ES_URL?has_content>
  <@img.TASK 'logstash-sc-web-${namespace}' 'imagenarium/logstash:7.17.1'>
  <@img.NODE_REF 'sc-web' />
    ...
    <@img.ENV 'NODE_NAME' 'logstash-sc-web-${namespace}' />
    <@img.ENV 'NUMBER_OF_REPLICAS' '0' />
    <@img.ENV 'INDEX_NAME' 'sc-web' />
